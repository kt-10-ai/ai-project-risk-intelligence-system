...
const STAGES = [
    ...
    { at: 20, label: 'Fetching Project Data' },
    { at: 45, label: 'Running Risk Models' },
    { at: 70, label: 'Aggregating Signals' },
...
export default function LoginPage() {
    ...
    const [statusLabel, setStatusLabel] = useState('Authenticating');
    ...
    function handleLogin(e: React.FormEvent) {
        ...
        const stored = JSON.parse(localStorage.getItem('meridian_user') || '{}');

        // Allow any stored user OR demo credentials
        const valid =
            (stored.email === email && stored.password === password) ||
            (email === 'admin@meridian.ai' && password === 'meridian2024');
                ...
                <div className="relative bg-[#12121a] border border-[#1e1e2d] rounded-xl shadow-2xl overflow-hidden">
                    <div className="p-8 pb-6">
                        ...
                        <div className="text-center mb-8">
                            <h2 className="text-slate-200 text-lg font-semibold">Welcome back</h2>
                        ...
                        <form className="space-y-5" onSubmit={handleLogin}>
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center">
                                    ...
                                    <a className="text-xs font-medium text-[#6764f2] hover:text-[#6764f2]/80 transition-colors" href="#">Forgot password?</a>
                                </div>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                        ...
                        <div className="relative mt-8">
                            <div className="absolute inset-0 flex items-center">
                                <div className="w-full border-t border-[#1e1e2d]" />
                            </div>
                            <div className="relative flex justify-center text-xs">
                                <span className="px-2 bg-[#12121a] text-slate-500">Or continue with</span>
                            </div>
                        </div>
                        <div className="mt-6 grid grid-cols-2 gap-3">
                            {['Google', 'GitHub'].map(p => (
                                <button key={p} type="button"
                                    className="w-full inline-flex justify-center items-center py-2.5 px-4 border border-[#1e1e2d] rounded-lg bg-[#1c1c27] text-sm font-medium text-slate-300 hover:bg-[#252532] transition-colors gap-2">
                                    <span className="material-symbols-outlined text-[18px]">{p === 'Google' ? 'language' : 'code'}</span>
                                    {p}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="bg-[#0f0f1a] px-8 py-4 border-t border-[#1e1e2d] flex justify-center">
                        <p className="text-sm text-slate-500">
                            ...
                            <Link className="font-medium text-[#6764f2] hover:text-[#6764f2]/80 transition-colors" to="/signup">
                                Sign up
                            </Link>
                        </p>
                    </div>
